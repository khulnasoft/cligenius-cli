...
def test_script_bye():
    result = subprocess.run(
        ["coverage", "run", "-m", "types_cli", "tests/assets/sample.py", "run", "bye"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        encoding="utf-8",
    )
...
def test_script_help():
    result = subprocess.run(
        ["coverage", "run", "-m", "types_cli", "tests/assets/sample.py", "--help"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        encoding="utf-8",
    )
...
def test_not_python():
    result = subprocess.run(
        ["coverage", "run", "-m", "types_cli", "tests/assets/not_python.txt", "run"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        encoding="utf-8",
    )
